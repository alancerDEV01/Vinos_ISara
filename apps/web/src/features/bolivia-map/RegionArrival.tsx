...
export function RegionArrival({ department, mode }: { department: string; mode: "vinos" | "platos" }) {
  ...
  return <section className="regionArrival" aria-label={`Paisajes de ${department}`}>
    <div className="regionArrivalCopy"><small>Departamento</small><h2>{department}</h2><p>{mode === "vinos" ? "Viñedos, cepas y perfiles sensoriales conectados con su paisaje." : "Cocina, ingredientes y memoria cultural conectados con su territorio."}</p></div>
